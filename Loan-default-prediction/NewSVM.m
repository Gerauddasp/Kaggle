...
% change labels
newlabels = labels;
addMatLabel(addMatLabel<bench) = -10;
addMatLabel(addMatLabel>bench) = -1;
addMatLabel(addMatLabel<-1) = 1;

newlabels(labels<bench)= -10;
newlabels(labels>bench) = -1;
newlabels(labels<-1)=1;
...
FN = (nnz(prediction(TestLabels == 1) ~= (TestLabels(TestLabels== 1)))/nnz(TestLabels==1))*100;

fprintf('\nTrue Positive: %2.1f\nFalse Negative: %2.1f\n', TP, FN);
fprintf('\nTrain\nNumber of defaults: %i\nNumber of non-default: %i\n',nnz(TrainLabels==-1),nnz(TrainLabels==1));
fprintf('\nTest\nNumber of defaults: %i\nNumber of non-default: %i\n',nnz(TestLabels==-1),nnz(TestLabels==1));

fprintf('\nTrue FN: %2.1f\n', (nnz(prediction(TrueTestLabels == 1) == TrueTestLabels(TrueTestLabels==1))/nnz(TrueTestLabels==1))*100);